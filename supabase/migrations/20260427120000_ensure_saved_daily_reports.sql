/*
  # Ensure daily report archive exists (idempotent).
  # Repairs databases where migration 20260424180000 is recorded but objects are missing.
*/
...
DROP POLICY IF EXISTS "Authenticated users can select saved_daily_reports" ON saved_daily_reports;
CREATE POLICY "Authenticated users can select saved_daily_reports"
  ON saved_daily_reports FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can insert saved_daily_reports" ON saved_daily_reports;
CREATE POLICY "Authenticated users can insert saved_daily_reports"
  ON saved_daily_reports FOR INSERT
  TO authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Authenticated users can delete saved_daily_reports" ON saved_daily_reports;
...
DROP POLICY IF EXISTS "job-reports bucket read" ON storage.objects;
CREATE POLICY "job-reports bucket read"
  ON storage.objects FOR SELECT
  TO authenticated
  USING (bucket_id = 'job-reports');

DROP POLICY IF EXISTS "job-reports bucket insert" ON storage.objects;
CREATE POLICY "job-reports bucket insert"
  ON storage.objects FOR INSERT
  TO authenticated
  WITH CHECK (bucket_id = 'job-reports');

DROP POLICY IF EXISTS "job-reports bucket update" ON storage.objects;
...
DROP POLICY IF EXISTS "job-reports bucket delete" ON storage.objects;
